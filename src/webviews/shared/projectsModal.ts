...
export class ProjectsModal {
    ...
    public static updateTranslations() {
        ...
        const titleEl = this.container.querySelector('.feedback-header h2');
        if (titleEl) titleEl.textContent = I18n.t('projects.title', 'Other Open Source Projects');
        const descEl = this.container.querySelector('.feedback-body p');
        if (descEl) descEl.textContent = I18n.t('projects.desc', 'Check out some of my other open-source projects on GitHub:');
        const closeBtn = this.container.querySelector('.feedback-close');
